...
class PandasIndicators:
    def __init__(self, df):
        n = 10
        df = self.moving_average(df, n)
        df = self.exponential_moving_average_ewm(df, n)
        df = self.exponential_moving_average_momentum(df, n)
        df = self.rate_of_change(df, n)
        df = self.average_true_range(df, n)
        df = self.bollinger_bands(df, n)
        df = self.ppsr(df, n)
        df = self.stochastic_oscillator_k(df)
        df = self.stochastic_oscillator_d(df, n)
        df = self.trix(df, n)
        df = self.average_directional_movement_index(df, n , n_ADX)
        df = self.macd(df, n_fast, n_slow)
        df = self.mass_index(df)
        df = self.vortex_indicator(df)
        df = self.kst_oscillator(df)
        df = self.relative_strength_index(df)
        df = self.true_strength_index(df)
        df = self.accumulation_distribution(df)
        df = self.chaikin_oscillator(df)
        df = self.money_flow_index(df)
        df = self.on_balance_volume(df)
        df = self.force_index(df)
        df = self.ease_of_movement(df)
        df = self.commodity_channel_index(df)
        df = self.coppock_curve(df)
        df = self.keltner_channel(df)
        df = self.keltner_channel(df)
        df = self.ultimate_oscillator(df)
        df = self.donchian_channel(df)
        df = self.standard_deviation(df)
    ...
    @staticmethod
    def exponential_moving_average_ewm(df, n):
        """
    ...
    @staticmethod
    def exponential_moving_average_momentum(df, n):
        """
